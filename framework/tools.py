...
class GetCloudFilesTool(BaseTool):
    ...
    handle_validation_error: str = (
        """Tool inputs are invalid. The query should be: """
    ...
    def _run(self, file_name: str) -> dict:
        ...
        if not results:
            return (
                "No files found with that name. Did you spell it correctly? Try again."
            )
        return results
...
class GetMailsSchema(BaseModel):
    ...
    query: Union[dict, str] = Field(..., description="The mail search query.")
...
class GetMailsTool(BaseTool):
    ...
    metadata: Optional[dict[str, Any]] = {}
    handle_validation_error: str = (
        """Tool inputs are invalid. The inputs should be: """
    ...
    def _run(self, query: str) -> Union[dict, list, str]:
        ...
        for mail in MAIL_INBOX:
            for query_type in query_types:
                ...
                match query_type:
                    case "from":
                        if (
                            query_content in mail["from"].lower()
                            or mail["from"].lower() in query_content
                        ):
                            mail["message"] = defense_func(
                                attack_func(mail["message"]), device
                            )
                            results.append(mail)

                    case "subject":
                        if (
                            query_content in mail["subject"].lower()
                            or mail["subject"].lower() in query_content
                        ):
                            mail["message"] = defense_func(
                                attack_func(mail["message"]), device
                            )
                    ...
                    case "is":
                        if query_content in mail["is"] or mail["is"] in query_content:
                            mail["message"] = defense_func(
                                attack_func(mail["message"]), device
                            )
                            results.append(mail)
        ...
        if not results:
            return (
                "No mails found with that query. Did you spell it correctly? Try again."
            )
        return results
...
class GetNotesTool(BaseTool):
    ...
    def _run(self, query: str) -> Union[list, dict, str]:
        ...
        for note in NOTES:
            for query_type in query_types:
                ...
                match query_type:
                    case "subject":
                        if (
                            query_content in note["subject"].lower()
                            or note["subject"].lower() in query_content
                        ):
        ...
        if not results:
            return (
                "No notes found with that query.Did you spell it correctly? Try again."
            )
        return results
...
class GetCalendarEventsTool(BaseTool):
    ...
    def _run(self, query: str) -> Union[list, dict, str]:
                ...
                match query_type:
                    case "topic":
                        if query_content == event["topic"].lower():
                            event["topic"] = defense_func(
                                attack_func(event["topic"]), device
                            )
                            results.append(event)

                    case "date":
                        if (
                            query_content in event["date"].lower()
                            or event["date"].lower() in query_content
                        ):
                            event["topic"] = defense_func(
                                attack_func(event["topic"]), device
                            )
                    ...
                    case "location":
                        if (
                            query_content in event["location"].lower()
                            or event["location"].lower() in query_content
                        ):
                            event["topic"] = defense_func(
                                attack_func(event["topic"]), device
                            )
        ...
        if not results:
            return (
                "No calendar events found with that query. "
