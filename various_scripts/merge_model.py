...
def main(base_llm: str, finetuned_model: str, quantization: bool) -> None:
    ...
    print(
        f"## {TColors.OKBLUE}{TColors.BOLD}Adapter/PEFT Model{TColors.ENDC}: {adapter_name}"
    )
    print(f"## {TColors.OKBLUE}{TColors.BOLD}Save Path:{TColors.ENDC}: {save_path}")
    print(
        f"## {TColors.OKBLUE}{TColors.BOLD}Quantization{TColors.ENDC}: {quantization}"
    )
    print("#" * os.get_terminal_size().columns + "\n")

    base_tokenizer = AutoTokenizer.from_pretrained(base_llm, use_fast=False)
    ...
    merged_model.save_pretrained(save_path, safe_serialization=True, save_adapter=True)
    ...
    print(
        f"=> {TColors.OKBLUE}{TColors.BOLD}Merged Model saved to:{TColors.ENDC}: {save_path}"
    )
